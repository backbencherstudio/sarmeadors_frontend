...
import { useUpdateChecklistBulkMutation } from "@/feature/slice/settings/candidates/CandidateSettingsSlice";
...
export function UpdateChecklistModal({ checklistItems }: UpdateChecklistModalProps) {
  const [updateChecklistBulk, { isLoading: isSaving }] =
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasChanges) {
      toast.error("Please ensure all checklist names are filled.");
      return;
    }

    try {
      await updateChecklistBulk(updates).unwrap();
      toast.success("Checklist updated successfully!");
      setOpen(false);
    } catch (error: any) {
      toast.error(
        error?.data?.message || "Error updating checklist. Please try again.",
      );
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription className="text-sm text-gray-600 pt-2 text-start">
              Edit checklist names and toggle status. Changes are saved in bulk.
            </DialogDescription>
          </DialogHeader>
