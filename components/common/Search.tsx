...
export default function Search() {
    ...
    const fetchData = async () => {
      try {
        ...
        const demoProducts = {
          data: {
            results: [
              { product: { name: "Maid Service Package A", category: { slug: "maid-services" } } },
              { product: { name: "Maid Service Package B", category: { slug: "maid-services" } } },
              { product: { name: "Cleaning Services", category: { slug: "cleaning" } } },
              { product: { name: "Household Management", category: { slug: "household" } } },
              { product: { name: "Care Services", category: { slug: "care-services" } } },
            ]
          }
        };
  ...
  <path d="M14.197 13.1363C13.9041 12.8434 13.4292 12.8434 13.1363 13.1363C12.8434 13.4292 12.8434 13.9041 13.1363 14.197L13.6667 13.6667L14.197 13.1363ZM13.6667 13.6667L13.1363 14.197L16.8863 17.947L17.4167 17.4167L17.947 16.8863L14.197 13.1363L13.6667 13.6667ZM15.75 8.25H16.5C16.5 3.69365 12.8063 0 8.25 0V0.75V1.5C11.9779 1.5 15 4.52208 15 8.25H15.75ZM8.25 0.75V0C3.69365 0 0 3.69365 0 8.25H0.75H1.5C1.5 4.52208 4.52208 1.5 8.25 1.5V0.75ZM0.75 8.25H0C0 12.8063 3.69365 16.5 8.25 16.5V15.75V15C4.52208 15 1.5 11.9779 1.5 8.25H0.75ZM8.25 15.75V16.5C12.8063 16.5 16.5 12.8063 16.5 8.25H15.75H15C15 11.9779 11.9779 15 8.25 15V15.75Z" fill="#111927"/>
</svg>
      </button>
        ...
        <div className="mt-4 bg-white shadow-md rounded-lg px-3 py-6 absolute w-full">
          {filteredProducts.length > 0 ? (
            filteredProducts.map((item: any, index: number) => (
              <button
                key={index}
                onClick={() => handleProductNameSearch(item.product.category.slug, item.product.name)}
            ...
            <p className="text-textColor p-2 flex justify-center font-semibold items-center gap-3"><CircleHelp className="text-primaryColor" size={40} /> No matching products found.</p>
          )}
        </div>
      )}
    </div>
